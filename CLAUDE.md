...
- **Pendente**: unificar os botões "Salvar" e "Concluir [etapa]" do `ModalAvaliacao.tsx` num só — hoje são ações separadas (`handleSave` vs `handleAvancarStatus`) e isso já causou confusão real (funcionário preenche tudo, clica só "Salvar", nunca clica "Concluir", e depois acha que perdeu o trabalho porque não aparece como concluída). Ainda não implementado, só especificado.
